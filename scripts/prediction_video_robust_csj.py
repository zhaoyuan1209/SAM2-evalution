...
from clicker import evaluate_sample_sam,evaluate_sample_sam_robust
import gc
import torch
        ...
        for t, tgt in zip(targets, targetsgt):
            ...
            if args.mode == 'auto':
                ...
                if len(bbox) != 0:
                    ...
                    bboxs = []
                    for x in bbox:
                        temp = np.array([max(0, x[0] + change_width), max(0, x[1] + change_height),
                                         min(gt.shape[0], x[2] + change_width), min(gt.shape[1], x[3] + change_height)])
                        bboxs.append(temp)
                ...
                final_mask,_ = evaluate_sample_sam_robust(image=image, gt_mask=gt, predictor=generator, max_iou_thr=0.9,
                                                 max_clicks=6,robust=True)
                final_mask = (final_mask.astype(np.uint8)) * 255
                save_images(t, image, gt, final_mask, None,args)
        # return
    print("Done!")
