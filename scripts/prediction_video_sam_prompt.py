...
def main(args: argparse.Namespace) -> None:
    ...
    for video, videogt in tqdm.tqdm(list(zip(video_list, video_gt_list))):
        targets = [
        ...
        for t, tgt in zip(targets, targetsgt):
            ...
            if args.mode == 'auto':
                ...
                prompt_mask = find_connect_area(prompt_mask)
                final_mask = (final_mask.astype(np.uint8)) * 255
                save_images(t,image,gt, final_mask,None,args = args)
                # if args.dataset_name == 'DAVIS16':
                #     save_path = args.output + '/' + tgt.split('/')[-2] + '/' + tgt.split('/')[-1]
                #     os.makedirs(args.output + '/' + tgt.split('/')[-2], exist_ok=True)
                # else:
                ...
                if prompt_mask is None:
                    final_mask,_ = evaluate_sample_sam2_video(image=image, gt_mask=gt, predictor=generator, max_iou_thr=0.9,
                                                 max_clicks=6)
                else:
                    final_mask,_ = evaluate_sample_sam2_video(image=image, gt_mask=gt, predictor=generator, max_iou_thr=0.9, prompt_mask=prompt_mask,
                                                     max_clicks=6)
                prompt_mask = final_mask.astype(np.uint8)
                final_mask = (final_mask.astype(np.uint8)) * 255
                save_images(tgt, None, gt, final_mask, None,args = args)
                # if args.dataset_name == 'DAVIS16':
                #     save_path = args.output + '/' + tgt.split('/')[-2] + '/' + tgt.split('/')[-1]
                #     os.makedirs(args.output + '/' + tgt.split('/')[-2], exist_ok=True)
                # else:
