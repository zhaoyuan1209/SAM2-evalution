...
for datasets in ["mvtec"]:#"mvtec3d","mvtec","visa""btad"
    for  model in ["sam2-l","sam-l"]:
        for mode in ["point"]:#
            gt_list = glob.glob(f"{predict_path}/{datasets}/{model}_{mode}/logits/*.npy")
            classes = list(set(map(lambda x:x.split(os.path.sep)[-1].split("_")[0],gt_list)))#[:2]
            results = []
            for cls in classes:
                gts,pres = [],[]
                for p in tqdm.tqdm(glob.glob(f"{predict_path}/{datasets}/{model}_{mode}/logits/{cls}*.npy")):
                    pre = np.load(p)
                    gt_filename = p.split('/')[-1][:-7]
                    gt_path=os.path.join(f"{source_path}/{datasets}/gt/{gt_filename}.png")
                    gt = cv2.cvtColor(cv2.imread(gt_path),cv2.COLOR_BGR2GRAY)
                    gt[gt>0]=1
                    # pres.append(cv2.resize(pre,(H,W)))
                    pre = torch.tensor(pre)[None,None]
                    pre = F.interpolate(pre,(H,W)).numpy()
                    while pre.shape[0]==1:
                         pre = pre[0]
                    pres.append(pre)
