...
datasets_list = []#["../datasets/ISBI_2015"]
datasets_list.extend(glob.glob(f"{source_path}/brats/*_flair_*"))
total_res = {}
for i in range(0,5):
    for datasets in datasets_list:#"mvtec3d","mvtec","visa"
        dataset_name = datasets.split(os.path.sep)[-1]
        for  model in ["sam2-l"]:
            mode_list = ["bbox","point"] if model=="sam-l" else ["bbox","point"]
            for mode in mode_list:#
                prompt_nums = ["_single"] if model=="sam2-l" else [""]
                for prompt_num  in prompt_nums:

                    patients = []
                    datasets_second_path ="brats" if "brats"  in dataset_name else "."

                    pre_path_list = glob.glob(os.path.join(predict_path,datasets_second_path,dataset_name,"Robust_"+str(i)+"_"+model+"_"+mode+prompt_num,"pred_mask","*.png"))
                        #
                    ...
                    for patient in patients:
                        pre_paths  =glob.glob(os.path.join(predict_path,datasets_second_path, dataset_name, "Robust_"+str(i)+"_"+model+"_"+mode+prompt_num, "pred_mask",f"{patient}*"))
                    ...
                    log = 'seed：{} method_name: {} mode:{} prompt_num:{} dataset: {} Dice: {:.4f} '.format(
                        i, model,mode, prompt_num, dataset_name, np.mean(dices))
                    print(log)
                    res = total_res.get(f"{dataset_name}_{model}_{mode}",[])
                    res.append(np.mean(dices))
                    total_res[f"{dataset_name}_{model}_{mode}"]=res
for key in total_res.keys():
    data = np.array([total_res[key]])#[:,[0,1,4,5,7]]
    means = [np.round(data.mean(),4)]
    std = [np.round(data.std(),5)]
